...
class ChunkHeapMemoryArena {
 ...
 public:
  ...
  [[nodiscard]] std::size_t used() const noexcept {
    std::size_t total = 0;
    for (const auto& c : chunks_) total += c.offset;
    return total;
  }

  // Bytes still available in the current (last) chunk before the next extension.
  [[nodiscard]] std::size_t available() const noexcept {
    return chunks_.back().capacity - chunks_.back().offset;
  }

  // alignment must be a power of two and must not exceed alignof(std::max_align_t):
  // new std::byte[] only guarantees that alignment for the buffer start, so
  // std::align cannot satisfy a larger alignment request.
  ...
  // Discards all extension chunks and resets the initial chunk's offset to zero.
  void reset() noexcept {
...
class ChunkHeapArenaAllocator {
  ...
  template <typename U>
  friend class ChunkHeapArenaAllocator;
 ...
 public:
  ...
  ChunkHeapArenaAllocator& operator=(const ChunkHeapArenaAllocator&) noexcept = default;
  ...
  void deallocate(T* p, std::size_t n) noexcept {
    ...
    if constexpr (sizeof(T) >= sizeof(FreeNode)) {
      if (n == 1) {
        FreeNode* node = ::new (static_cast<void*>(p)) FreeNode{*free_list_head_};
